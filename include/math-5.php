...
<?php 
	...
	if(isset($_REQUEST['type']) && $_REQUEST['type'] === 't'){
		...
		for($i=1;$i<=$bookCount;$i++)
		{
			$modelClass = 'm-m-ins-5-'.$i;
			$imagePath = 'image/m/instructor/5/'.$i.'.jpg';
			$bookName = $bookNames[($i - 1)];  /* --- ใช้ ($i-1) เพราะว่า array เริ่มต้นด้วย index ที่ 0 --- */
			$nameOfComponent = array('r_m_ins_5_'.$i.'_1', 'r_m_ins_5_'.$i.'_2', 't_m_ins_5_'.$i.'_3', 'f_m_ins_5_'.$i.'_4', 'h_m_ins_5_'.$i.'_4');
			$collapseSectionId = 'mMIns5'.$i.'SelectedCollapse';
			$fileUploadId = 'fMIns5'.$i.'4';
