...
<?php 
	...
	{
		$modelClass = 'm-m-7-'.$i;
		$imagePath = 'image/m/7/'.$i.'.jpg';
		$bookName = $bookNames[($i - 1)];  /* --- ใช้ ($i-1) เพราะว่า array เริ่มต้นด้วย index ที่ 0 --- */
		$nameOfComponent = array('r_m_7_'.$i.'_1', 'r_m_7_'.$i.'_2', 't_m_7_'.$i.'_3', 'f_m_7_'.$i.'_4', 'h_m_7_'.$i.'_4');
		$collapseSectionId = 'mM7'.$i.'SelectedCollapse';
		$fileUploadId = 'fM7'.$i.'4';
	...
	if(isset($_REQUEST['type']) && $_REQUEST['type'] === 't'){
		...
		for($i=1;$i<=$bookCount;$i++)
		{
			$modelClass = 'm-m-ins-7-'.$i;
			$imagePath = 'image/m/instructor/7/'.$i.'.jpg';
			$bookName = $bookNames[($i - 1)];  /* --- ใช้ ($i-1) เพราะว่า array เริ่มต้นด้วย index ที่ 0 --- */
			$nameOfComponent = array('r_m_ins_7_'.$i.'_1', 'r_m_ins_7_'.$i.'_2', 't_m_ins_7_'.$i.'_3', 'f_m_ins_7_'.$i.'_4', 'h_m_ins_7_'.$i.'_4');
			$collapseSectionId = 'mMIns7'.$i.'SelectedCollapse';
			$fileUploadId = 'fMIns7'.$i.'4';
