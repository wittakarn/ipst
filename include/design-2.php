...
<?php 
	...
	if(isset($_REQUEST['type']) && $_REQUEST['type'] === 't'){
		...
		for($i=1;$i<=$bookCount;$i++)
		{
			$modelClass = 'm-d-ins-2-'.$i;
			$imagePath = 'image/d/instructor/2/'.$i.'.jpg';
			$bookName = $bookNames[($i - 1)];  /* --- ใช้ ($i-1) เพราะว่า array เริ่มต้นด้วย index ที่ 0 --- */
			$nameOfComponent = array('r_d_ins_2_'.$i.'_1', 'r_d_ins_2_'.$i.'_2', 't_d_ins_2_'.$i.'_3', 'f_d_ins_2_'.$i.'_4', 'h_d_ins_2_'.$i.'_4');
			$collapseSectionId = 'mdIns2'.$i.'SelectedCollapse';
			$fileUploadId = 'fdIns2'.$i.'4';
