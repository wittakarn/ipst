...
<?php 
	...
	if(isset($_REQUEST['type']) && $_REQUEST['type'] === 't'){
		...
		for($i=1;$i<=$bookCount;$i++)
		{
			$modelClass = 'm-s-ins-2-'.$i;
			$imagePath = 'image/s/instructor/2/'.$i.'.jpg';
			$bookName = $bookNames[($i - 1)];  /* --- ใช้ ($i-1) เพราะว่า array เริ่มต้นด้วย index ที่ 0 --- */
			$nameOfComponent = array('r_s_ins_2_'.$i.'_1', 'r_s_ins_2_'.$i.'_2', 't_s_ins_2_'.$i.'_3', 'f_s_ins_2_'.$i.'_4', 'h_s_ins_2_'.$i.'_4');
			$collapseSectionId = 'msIns2'.$i.'SelectedCollapse';
			$fileUploadId = 'fsIns2'.$i.'4';
