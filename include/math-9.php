...
<?php 
	...
	if(isset($_REQUEST['type']) && $_REQUEST['type'] === 't'){
		...
		for($i=1;$i<=$bookCount;$i++)
		{
			$modelClass = 'm-m-ins-9-'.$i;
			$imagePath = 'image/m/instructor/9/'.$i.'.jpg';
			$bookName = $bookNames[($i - 1)];  /* --- ใช้ ($i-1) เพราะว่า array เริ่มต้นด้วย index ที่ 0 --- */
			$nameOfComponent = array('r_m_ins_9_'.$i.'_1', 'r_m_ins_9_'.$i.'_2', 't_m_ins_9_'.$i.'_3', 'f_m_ins_9_'.$i.'_4', 'h_m_ins_9_'.$i.'_4');
			$collapseSectionId = 'mMIns9'.$i.'SelectedCollapse';
			$fileUploadId = 'fMIns9'.$i.'4';
