...
<?php 
	...
	if(isset($_REQUEST['type']) && $_REQUEST['type'] === 't'){
		...
		for($i=1;$i<=$bookCount;$i++)
		{
			$modelClass = 'm-s-ins-4-'.$i;
			$imagePath = 'image/s/instructor/4/'.$i.'.jpg';
			$bookName = $bookNames[($i - 1)];  /* --- ใช้ ($i-1) เพราะว่า array เริ่มต้นด้วย index ที่ 0 --- */
			$nameOfComponent = array('r_s_ins_4_'.$i.'_1', 'r_s_ins_4_'.$i.'_2', 't_s_ins_4_'.$i.'_3', 'f_s_ins_4_'.$i.'_4', 'h_s_ins_4_'.$i.'_4');
			$collapseSectionId = 'msIns4'.$i.'SelectedCollapse';
			$fileUploadId = 'fsIns4'.$i.'4';
