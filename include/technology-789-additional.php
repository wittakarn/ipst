...
<?php 
	...
	if(isset($_REQUEST['type']) && $_REQUEST['type'] === 't'){
		...
		for($i=1;$i<=$bookCount;$i++)
		{
			$modelClass = 'm-t-ins-789-additional-'.$i;
			$imagePath = 'image/t/instructor/789-additional/'.$i.'.jpg';
			$bookName = $bookNames[($i - 1)];  /* --- ใช้ ($i-1) เพราะว่า array เริ่มต้นด้วย index ที่ 0 --- */
			$nameOfComponent = array('r_t_ins_789_additional_'.$i.'_1', 'r_t_ins_789_additional_'.$i.'_2', 't_t_ins_789_additional_'.$i.'_3', 'f_t_ins_789_additional_'.$i.'_4', 'h_t_ins_789_additional_'.$i.'_4');
			$collapseSectionId = 'mtIns789Additional'.$i.'SelectedCollapse';
			$fileUploadId = 'ftIns789Additional'.$i.'4';
